...
To derive a [sos2](/command/sos2) representation, we first focus on representing the border of the polygon. With given vertices \\(v_i = (x_i,y_i)\\), every point on the border of the star can be written as a linear combination of two adjacent vertices. This can be written as \\(V\lambda\\) where where \\(V\\) are the stacked vertices and where \\(lambda\\) is a non-negative vector with at most two adjacent non-zero elements summing up to \\( 1\\). This is the classical application of [sos2](/command/sos2), and precisely the same model YALMIP uses for general [interp1](/command/interp1) representation.
...
% Now say that each point in point-cloud is in starpolygon
sdpvar t
c = sdpvar(2,1);
Model = [];
for i = 1:length(xn)
