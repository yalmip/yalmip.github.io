...
````
Once again, we implement this using a combination of YALMIP and [MPT] commands
````matlab
plot([ismember(x+w,P), ismember(w,S), uncertain(w)]);
````

Note that the use of the [robust optimization framework] in YALMIP allows us to generalize the Minkowski difference to subtract an arbitrary conic set. [MPT] on the other hand is limited to polytopic sets.
...
Addition of polytopes, i.e. Minkowski sum, is also directly available in [MPT]
...
[MPT] has support for slicing. This command allows you to fix the value of a set of variables, and plot the polytope in the remaining free variables. Let us fix the first coordinate to the value 0.2 in the 3D polytope above
