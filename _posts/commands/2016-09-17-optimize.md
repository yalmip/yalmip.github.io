...
A diagnostic structure is returned from solvesdp, which can be used, e.g, to check feasibility as reported by the solver (see [yalmiperror](/command/yalmiperror) for the error codes)
...
Solving the feasibility problem with a particular solver, e.g. [QUADPROG](/solver/quadprog), can be done by creating an options structure
