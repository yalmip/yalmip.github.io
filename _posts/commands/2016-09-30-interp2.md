...
[interp2](/command/interp2) overloads and extends interp2
...
Almost surely, a nonlinear solver will fail to find a good solution, but gets stuck in the origin. The same happens when we simply use the nonlinear function directly
...
As an alternative way to attack this with a hope of a global solution is to descibe the objective using a  [sos2](/command/sos2) representation and thus solve the problem using a  [mixed-integer linear solver](/tag#mixed-integer-linear-programming-solver). 
