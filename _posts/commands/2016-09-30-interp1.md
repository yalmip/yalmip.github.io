...
Of course, we could have worked with the nonlinear function directly.
...
For problems where the data represents a convex (or concave) function and convexity of the problem can be derived, we can more efficiently use a simple linear epi-graph (hypo-graph) representation. This is obtained with the flag **'graph'**. YALMIP will automatically analyze the problem to check convexity
