...
The problem we apply [kkt](/command/kkt) to must be an LP or QP in the decision variables. However, it can be more complex w.r.t the parameters. The following examples illustrates this by solving a nonlinear [/tutorial/bilevelprogramming/](bilevel program), by deriving the nonlinearly parameterized KKT conditions of the inner program. The resulting problem will inherit the nonconvex quadratic structure from the initial models, and have additional nonconvex complementarity constraints arising from the KKT conditions, but is readily solved using a global quadratic solver such as [/solver/bmibnb](bmibnb)
