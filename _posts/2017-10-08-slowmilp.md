...
tags: [Performance, Mixed-integer linear programming]
comments: true
date: '2017-10-08'
---
...
In the worst case, to solve a MILP with 1000 variables, we will have to go through all possible cases, i.e. \\(2^{1000}\\). How large is\\(2^{1000}\\) compared to \\(4.32\cdot 10^{21}\\)? Absolutely enormously uncomparably larger. Already \\(2^{72}\\) is larger than  \\(4.32\cdot 10^{21}\\), i.e., with our solver we might, in the worst case, only solve a problem with 72 binary variables if we are given the age of the universe to finish. For a problem with 1000 variables, the worst-case required time is absolutely incomprehensible.
